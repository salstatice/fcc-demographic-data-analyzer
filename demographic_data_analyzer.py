...
def calculate_demographic_data(print_data=True):
    ...
    rich_percentage = round(num_min_workers['salary'].value_counts()['>50K'] / len(num_min_workers) * 100, 1)

    # What country has the highest percentage of people that earn >50K?

    # referencing
    # https://forum.freecodecamp.org/t/pandas-easier-way-of-using-groupby/414164
    df['>50K-flag'] = df['salary'] == '>50K'

    highest_earning_country = df.groupby('native-country').mean()['>50K-flag'].idxmax()
    highest_earning_country_percentage = round(df.groupby('native-country').mean()['>50K-flag'].max() * 100, 1)

    # Identify the most popular occupation for those who earn >50K in India.
    rich_india = df[(df['native-country']=='India') & (df['salary'] == '>50K')]
    top_IN_occupation = rich_india['occupation'].value_counts().index.tolist()[0]
